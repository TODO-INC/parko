...
# @app.get('/get_user_booking')
# def get_booking(request: Request, db: Session = Depends(get_db)):
#     user_data = db.query(model.register).filter(model.register.status=="ACTIVE").all()
#     return user_templates.TemplateResponse("booking.php", context={"request": request,"data":user_data})
...
@app.post('/post_register/{user_id}')
def create_data(user_id:str,request:Request,db:Session=Depends(get_db),host_id:str=Form(...),vehicleType:str=Form(...),vehicleNumber:str=Form(...),startDate:str=Form(...),startTime :str=Form(...),endDate:str=Form(...),endTime:str=Form(...),cost:float=Form(...)):
    statuss="ACTIVE"
    created_at = current_date_time
    updated=" "
    s=" "
    sss=" "
    ss=0
    start_datetime = datetime.strptime(f'{startDate} {startTime}', '%Y-%m-%d %H:%M')
    end_datetime = datetime.strptime(f'{endDate} {endTime}', '%Y-%m-%d %H:%M')
    body1 = db.query(model.host).filter(model.host.host_id==host_id).first()
    body1.income += cost
    db.commit()

    body=model.register(user_id=user_id,host_id=host_id,vehicleType=vehicleType,vehicleNumber=vehicleNumber,startDate=startDate,startTime=start_datetime,endDate=endDate,endTime=end_datetime,cost=cost,status=statuss,created_at=created_at,updated_at=updated,booking_status=ss,rating=s,review=sss)
    db.add(body)
    body2 = db.query(model.sign_up).filter(model.sign_up.user_id==user_id).first()
    body2.wallet-=cost
    db.add(body2)
    db.commit()
    return RedirectResponse(f"/get_user_booking/{user_id}", status_code=303)



@app.post('/post_extend/{user_id}')
def create_data(user_id:str,request:Request,db:Session=Depends(get_db),id:int=Form(...),userInput : int=Form(...),calculatedValue:int=Form(...)):
    statuss="ACTIVE"
    updated_at=current_date_time
    print(user_id)
    user_data=db.query(model.register).filter(model.register.id==id).first()
    new_end_time = user_data.endTime + timedelta(hours=userInput)
    
    # Get the current datetime
    current_datetime = datetime.now()
    
    # Compare new_end_time with the current time plus a day
    if new_end_time >= (current_datetime + timedelta(days=1)):
        days_to_add = (new_end_time - current_datetime).days
        new_end_date = user_data.endDate + timedelta(days=days_to_add)
        new_end_time -= timedelta(days=days_to_add)
    else:
        new_end_date = user_data.endDate
    
    # Update user_data
    user_data.endTime = new_end_time
    user_data.endDate = new_end_date
    
    # Commit changes to the database
    db.commit()
    l=user_data.cost+calculatedValue

    
    db.query(model.register).filter(model.register.id==id).update({"updated_at":updated_at,"cost":l})
    db.commit()
    
    return RedirectResponse(f"/get_user_booking/{user_id}", status_code=303)


@app.post('/post_review/{user_id}')
def create_data(user_id:str,request:Request,db:Session=Depends(get_db),re_id:int=Form(...),reviewRating : str=Form(...),review:str=Form(...)):
    statuss="ACTIVE"
    updated_at=current_date_time
    print(user_id)
    db.query(model.register).filter(model.register.id==re_id).update({"updated_at":updated_at,"rating":reviewRating,"review":review})
    db.commit()
